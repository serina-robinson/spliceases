...
joindf1$e_value3 <- as.character(joindf1$e_value3)
...
#cybr <- ygadf %>%
#  dplyr::filter(grepl("Cystobacter", genus_species)) # Cystobacter fuscus is there...
#write_csv(cybr, "output/cystobacter_unfiltered.csv")
...
ygabar <- ygalong %>%
  dplyr::mutate(prot_var = paste0(protein_acc, "_", variable)) %>%
  dplyr::distinct() %>%
  group_by(query_acc) %>%
  dplyr::add_count() %>%
...
write_csv(ygabar, "data/XYG_count_data/unfiltered_combined_YG_YA_count_data.csv")
